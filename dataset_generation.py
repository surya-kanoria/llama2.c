    ...
    with torch.no_grad():
        with ctx:
            for k in range(num_samples):
                ...
                story1 = (enc.decode(y[0].tolist()))
                y = model.generate(x, max_new_tokens, temperature=temperature, top_k=top_k)
                story2 = (enc.decode(y[0].tolist()))
                all_stories.append({
                    "story1": story1,
                    "story2": story2
                })
            if i % 100 == 0:
                print("Generating file: ", file_num)
                file_name = os.path.join(data_dir, f"dataset_{str(file_num)}.json")
                with open(file_name, "w") as f:
                    json.dump(all_stories, f)
                all_stories = []
                file_num = file_num + 1
